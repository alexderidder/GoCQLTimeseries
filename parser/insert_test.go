...
func TestInsertParseJSON(t *testing.T) {
	test := model.InsertJSON{}
	insert := Insert{[]byte{1, 0, 0, 0}, []byte("{\"stoneID\":\"bf82e78d-24a2-470d-abb8-9e0a2720619f\",\"data\":[{\"time\":\"2018-11-14T10:16:26.7220151+01:00\",\"kWh\":3.3228004,\"watt\":3.0233014,\"pf\":4.702545}]}"), &test}
	flag1 := InsertFlag1{&insert}
...
func TestInsertRequirements(t *testing.T) {
	...
	test = model.InsertJSON{}
	insert = Insert{[]byte{1, 0, 0, 0}, []byte("{\"data\":[{\"time\":\"2018-11-14T10:16:26.7220151+01:00\",\"kWh\":3.3228004,\"watt\":3.0233014,\"pf\":4.702545}]}"), &test}
	assert.Equal(t, true, flag1.marshalBytes(), "Marshal byte went wrong")
	assert.Equal(t, model.MissingStoneID, flag1.checkParameters(), "Check if stone ID is missing")

	test = model.InsertJSON{}
	insert = Insert{[]byte{1, 0, 0, 0}, []byte("{\"stoneID\":\"bf82e78d-24a2-470d-abb8-9e0a2720619f\"}"), &test}
	assert.Equal(t, true, flag1.marshalBytes(), "Marshal byte went wrong")
