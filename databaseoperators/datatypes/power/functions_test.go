...
func TestWithNoTimestampsFromDatabase(t *testing.T) {
	...
	lastValue := AggregatePower{timestamp:-1, }
	m.On("Scan", mock.Anything, mock.Anything, mock.Anything ).Return(false)
	...
	assert.Equal(t, AggregatePower{}, lastValue, "Last value is datapoint with time 0")
	assert.Equal(t, []datatypes.Data{}, aggregatedList, "Check if no value is added to aggregatedList")

	//lastValue = AggregatePower{timestamp:-1, }
	//lastValue.timestamp = 100
	//lastValue.sumPF = 150
	//lastValue.count = 15
